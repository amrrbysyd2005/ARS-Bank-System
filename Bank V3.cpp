...
stClient GetClientInput(vector<stClient>& Clients, const string& currentAccountNumber) {
    ...
    if (currentUserIt != Clients.end() && currentUserIt->Role == enUserRole::Manager) {
        Client.Role = GetRole();
    }
    else {
...
void PrintClientCard(const stClient& Client) {
    ...
    cout << " Role           : " << (Client.Role == enUserRole::Manager ? "Manager" : Client.Role == enUserRole::Employee ? "Employee" : "Customer") << "\n";
    cout << "========================================\n";
}
...
string ConvertClientToLine(const stClient& Client, const string& delimiter = "#//#") {
    return Client.AccountNumber + delimiter +
        Client.PinCode + delimiter +
        Client.FullName + delimiter +
        Client.Phone + delimiter +
        to_string(Client.Balance) + delimiter +
        (Client.Role == enUserRole::Manager ? "M" : Client.Role == enUserRole::Employee ? "E" : "C");
}

vector<string> SplitLineToVector(string line, const string& delimiter) {
    size_t pos = 0;
...
stClient ParseLineToClient(const vector<string>& vRecord) {
    ...
    if (vRecord.size() >= 6) {
        ...
        Client.Role = (vRecord.at(5) == "M" ? enUserRole::Manager : vRecord.at(5) == "E" ? enUserRole::Employee : enUserRole::Customer);
    }
    return Client;
}
...
void printWelcomeScreen(const string& bankNameParam) {
    system("cls");
    cout << "\n\n\n\n\n\n";
    ...
    int headLen = (int)("WELCOME TO " + bankNameParam + " BANK").length();
    int pad1 = (60 - 2 - headLen) / 2;
    cout << indent << "|";
    PrintSeparator(" ", pad1);
    cout << "WELCOME TO " << bankNameParam << " BANK";
...
DeletionChoice getDeletionConfirmation() {
    ...
    int choice = ReadIntWithMaxAndMinLimit("Enter choice (1-3):\t", 1, 3);
    switch (choice) {
    case 1: return DeletionChoice::SaveNow;
    case 2: return DeletionChoice::SaveLater;
    case 3: return DeletionChoice::Cancel;
    default: return DeletionChoice::SaveNow;
    }
}
...
void updateClient(vector<stClient>& vClients, const string& currentAccountNumber, bool& isDataChanged) {
    ...
    if (currentUserIt != vClients.end() && currentUserIt->Role == enUserRole::Manager) {
...
void DepositLogic(vector<stClient>& vClients, const string& accountNumber) {
    auto it = FindClientByAccountNumber(vClients, accountNumber);
    if (it == vClients.end()) return;
...
void ShowDepositScreen(vector<stClient>& vClients, const string& accountNumber) {
    system("cls");
    ...
    DepositLogic(vClients, accountNumber);

    it = FindClientByAccountNumber(vClients, accountNumber);
    cout << "\n--- Updated Client Details ---\n";
    PrintClientCard(*it);
}

void WithdrawLogic(vector<stClient>& vClients, const string& accountNumber) {
    auto it = FindClientByAccountNumber(vClients, accountNumber);
    if (it == vClients.end()) return;
    ...
    while (WithDraw <= 0 || WithDraw > it->Balance) {
        if (WithDraw <= 0) {
            cout << "[Error] Withdraw amount must be greater than zero!\n";
        }
        else if (WithDraw > it->Balance) {
...
void ShowWithDrawScreen(vector<stClient>& vClients, const string& accountNumber) {
    system("cls");
    ...
    WithdrawLogic(vClients, accountNumber);
...
void ShowTotalBalanceScreen(vector<stClient>& vClients, const string& accountNumber) {
    system("cls");
    ...
    cout << "\n Total Balance:\t\t$" << fixed << setprecision(2) << it->Balance << endl;
}
...
bool PerformTransactionProcess(vector<stClient>& vClients, const string& accountNumber, bool& isDataChanged) {
    ...
    case enTransactionProcess::Deposit:
        ShowDepositScreen(vClients, accountNumber);
        isDataChanged = true;
        GoBackToMainMenue();
        return false;
    case enTransactionProcess::Withdraw:
        ShowWithDrawScreen(vClients, accountNumber);
        isDataChanged = true;
        GoBackToMainMenue();
        return false;
    case enTransactionProcess::Total_Balance:
        ShowTotalBalanceScreen(vClients, accountNumber);
        GoBackToMainMenue();
        return false;
...
void ShowTransactionOnScreen(vector<stClient>& vClients, const string& accountNumber) {
    bool isDataChanged = false;
    while (true) {
        ShowTransactionMenu();
        bool Exit = PerformTransactionProcess(vClients, accountNumber, isDataChanged);
        if (Exit) {
            HandleSystemExit(vClients, isDataChanged);
            return;
        }
    }
}
...
bool PerformMainMenuOption(vector<stClient>& Clients, const string& currentAccountNumber, bool& isDataChanged) {
    auto currentUserIt = FindClientByAccountNumber(Clients, currentAccountNumber);
    if (currentUserIt == Clients.end()) return true;
...
void ShowSensitiveOperationsOnScreen(vector<stClient>& Clients, const string& currentAccountNumber) {
    bool isDataChanged = false;
    while (true) {
        ShowMainMenue();
...
void LoginPage(vector<stClient>& vClients) {
    ...
    string loggedInAccount = it->AccountNumber;
    ...
    while (remainingAttempts > 0) {
        ...
        it = FindClientByAccountNumber(vClients, loggedInAccount);
        if (it != vClients.end() && pinCode == it->PinCode) {
            if (it->Role == enUserRole::Customer) {
                ShowTransactionOnScreen(vClients, loggedInAccount);
            }
            else {
                ShowSensitiveOperationsOnScreen(vClients, loggedInAccount);
            }
            return;
        }
